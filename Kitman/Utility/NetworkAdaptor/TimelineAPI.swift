...
//  Swagafied
//
//  Created by Amitabha on 14/11/16.
...
class TimelineAPI: APIBase {
    
    var products: Array<TimelineProduct> = Array<TimelineProduct>()
    ...
    override var apiPath:String {
        ...
        var path = Configuration.API.Path.UserTimeline+"?"
        ...
        if let apiResponse =  response.result.value{
            ...
            if (response.result.error != nil) {
                ...
                if success == true{
                    ...
                    let responseLocalObject = Mapper<TimelineResponse>().map(JSON: responseObject)
