...
//  Swagafied
//
//  Created by Amitabha on 05/11/16.
...
class ProtfolioListAPI: APIBase {
    
    var dafaultUserID =  Utility.getUserID()
    var products: Array<Portfoliolist> = Array<Portfoliolist>()
    var message:String?
    ...
    override var apiPath:String {
        ...
        var path = Configuration.API.Path.Portfolio+"?"
        ...
        if let apiResponse =  response.result.value{
            ...
            if (response.result.error != nil) {
                ...
                if success == true{
                    ...
                    let responseLocalObject = Mapper<PortfolioResponse>().map(JSON: responseObject)
