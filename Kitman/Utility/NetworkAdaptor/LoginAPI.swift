...
class LoginAPI: APIBase {
    ...
    override var apiPath:String {
        var path = Configuration.API.Path.Login+"?"
        ...
        if let apiResponse =  response.result.value{
            ...
            if (response.result.error != nil) {
                ...
                if let isError = responseObject["success"] as? Bool{
                    ...
                    if success == false{
                        JSSAlertView().danger((sharedAppDelegate.window?.rootViewController?.presentedViewController)!, title: "Error", text: "Invalid Credentails.Please enter valid credentials")
                    }else{
                ...
                if success == true{
                    ...
                    UserDefaults.standard.set(userModel?.userEmailID, forKey: "userEmailID")
                }
                else{
